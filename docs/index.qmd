...
There are multiple ways to conduct the retraining procedure. We will consider the two following alternatives here:
...
- The two models predict the code given by the annotator with a rate of 0.71;
- The fastText model predict the code given by the annotator which is different from the code predicted by the CamemBERT model with a rate of 0.04;
- The CamemBERT model predict the code given by the annotator which is different from the code predicted by the fastText model with a rate of 0.05;
- The two models predict the same code but it is different from the annotation with a rate of 0.17.

[^4]: CamemBERT is a language model for French based on the RoBERTa architecture pretrained on the French subcorpus of the multilingual corpus OSCAR [@martin-etal-2020-camembert].
