...
The REST API is developed using FastAPI, a modern, high-performance, web framework for building APIs with Python. We encapsulate the API code and its dependencies into a Docker image, which is deployed as a container on a Kubernetes cluster. This grants the ability to easily scale the API according to demand thanks to automatic load-balancing. Upon startup, the API automatically retrieves the text classification model from storage. The model is packaged as an MLflow model, which integrates pre-processing steps in its inference method. This means that clients do not have to apply these steps themselves. This prevents many potential mistakes.  

# Monitoring {#sec-monitoring}

## Design

As explained in @sec-context, the activity of the codification engine is monitored in almost real-time. This monitoring is essential to detect anomalies in the inference data, or to be able to make decision on when to retrain the ML model behind the engine. The monitoring system is made up of two distinct blocks:

- A simple *Extract-Transform-Load* (ETL) process is run nightly from a Python script to fetch the logs recording the activity of the codification API. The logs are parsed and their content is saved as partitioned Parquet files;
- An interactive dashboard is built to offer insight on the data fed to the API and how it is coded.

The relevant computations are done with direct SQL queries on the Parquet files storing logs content using the DuckDB engine. The dashboard is also rebuilt nightly to integrate the latest data in its static visualizations. It is deployed as a container on a Kubernetes cluster.

## Engine monitoring

The monitoring dashboard offers insight about the codification on several levels:

- The daily and weekly number of queries made to the API are displayed in two separate graphs on a tab. The daily and weekly automatic codification rates are also displayed, in reactive visualizations which are modified according to the confidence threshold for automatic codification displayed by the user. The aggregate automatic codification rate on all data is also displayed (see @fig-tab1);
- The distribution of confidence intervals is displayed for a time window specified by the user (@fig-tab4);
- For a time window, a level of the NAF classification system and a confidence threshold specified by the user, the number of predictions in each possible class in displayed in a barplot, colored with the corresponding automatic codification rates for these classes (@fig-tab5);
...
- A first tab gives elements on the entire evaluation set, given a specified threshold on the confidence for automatic codification (@fig-tab2):
  - The total number of observations in the evaluation set;
  - The automatic coding rate;
  - The accuracy on automatically-coded descriptions;
  - The top-2 to top-5 accuracies on automatically-coded descriptions;
  - The accuracies on automatically-coded descriptions at all levels of the NAF classification system;
...
- A second tab gives monthly estimates for the accuracy of the codification engine, with bootstrap confidence intervals (@fig-tab3).

# Retraining {#sec-retraining}

## Triggers

"When" is one of the first questions arising when thinking about retraining the ML model behind the codification engine. As explained earlier, business activities evolve over time, so the codification engine should not be static. At a given time, the model in use should be trained with data which covers as much as possible the entire distribution of inference data. This means that if a new business appears at some point $t$ in time, the ML model used for codification should be retrained or fine-tuned with training data including data points corresponding to this new business. In theory, this will be the case for an extraction of the registry following $t$. Indeed, the data points in question should be given a low confidence score, and thus be manually assigned the correct code.  

It seems natural to advocate frequent retraining or fine-tuning procedures to avoid long periods of time where the data points mentioned above are all coded manually. However, it is good practice to have a thorough validation procedure when switching production models. This validation can be costly, which on the contrary encourages to re-train sparingly over time. This is all the more true that we are not dealing with extremely high-frequency data and that the emergence of new businesses is a relatively one-off phenomenon. All in all, it seems reasonable considering our validation procedure to periodically retrain our classification model every 3-6 months.

In addition to this periodic update, our monitoring allows us to trigger specific retraining procedures when certain events are observed -- for instance, as soon as the monthly accuracy computed on our evaluation set (see @fig-tab8) falls under a certain threshold.
...
Another situation mentioned earlier which can happen in the context of our codification use-case is important to describe in more detail: businesses traditionally associated with a certain activity may see this activity evolve. From a certain point on, the code which should be assigned to such a business changes from a code A to another code B. Regarding data points corresponding to this business that were coded manually, then directions given by manual annotators were hopefully modified accordingly. This means that in the registry data, data points corresponding to the same business will partly be labeled as A up to some point and as B later on. We would therefore like to retrain or fine-tune the ML model with data including the latest B labels as much as possible. If this is not  


- Retraining from scratch, going back to a certain depth ?
- Retraining only with a small lr from pretrained weights, with only more recent data points to adjust ?


# Discussion

- BERT vs. fastText: comparison analysis

# References

# Supplementary material

![Dashboard tab offering daily and weekly insight on the number of queries to the API and its automatic codification rate.](img/tab1.png){#fig-tab1}

![Dashboard tab displaying the distribution of confidence values for a specified time window.](img/tab4.png){#fig-tab4}

![Dashboard tab displaying numbers of queries and automatic codification rates for classes at a specified level of the classification system and for a specified time window.](img/tab5.png){#fig-tab5}
...
![Dashboard tab allowing users to look through the data sent to the codification engine within a specified time window.](img/tab7.png){#fig-tab7}
...
![Dashboard tab giving insight on the aggregate performance of the codification engine on the evaluation set.](img/tab2.png){#fig-tab2}

![Dashboard tab giving insight on the monthly accuracy of the codification set.](img/tab3.png){#fig-tab3}
